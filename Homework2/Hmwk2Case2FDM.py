...
#Hmwk 2 Case 2 FDM Solutions with error, beta, and total heat loss
...
def case_2_produce_FDE_matrix(n):
    ...
    # fourth order k
    #k_normal = 2+alpha**2*delta_x**2*(1 + delta_x**2/12*alpha**2)
    # sixth order k
    # k_normal = 2+alpha**2*delta_x**2*(1 + delta_x**2/12*alpha**2+delta_x**4/360*alpha**4)
    # eighth order k
    #k_normal = 2+alpha**2*delta_x**2*(1 + delta_x**2/12*alpha**2+delta_x**4/360*alpha**4+delta_x**6/20160*alpha**6)
    # tenth order k
    k_normal = 2+alpha**2*delta_x**2*(1 + delta_x**2/12*alpha**2+delta_x**4/360*alpha**4+delta_x**6/2160*alpha**6+delta_x**8/1814400*alpha**8)
    case_2_row_triple = [-1, k_normal, -1]

    #case 2 of heat transfer across rod with new coefficient
    # second order K'
    #k_prime = (1*delta_x*h/k+alpha**2*delta_x**2/2)
    # 4th order K'
    # k_prime = delta_x*h/k*(1+delta_x**2*alpha**2/6) + k_normal/2
    # 6th order K'
    #k_prime = delta_x*h/k*(1+delta_x**2*alpha**2/6+delta_x**4*alpha**4/120) + k_normal/2
    # 8th order K'
    # k_prime = delta_x*h/k*(1+delta_x**2*alpha**2/6+delta_x**4*alpha**4/120+delta_x**6*alpha**6/5040) + k_normal/2
    # 10th order K'
    k_prime = delta_x*h/k*(1+delta_x**2*alpha**2/6+delta_x**4*alpha**4/120+delta_x**6*alpha**6/5040+delta_x**8*alpha**8/362880) + k_normal/2

    #setting first row of second case to new matrix pair
    FDE_matrix[0][0:2] = [k_prime, -1]
...
for n in range(2, 13):
    ...
    print("Lateral heat flux values:")
    print('{:.12f}'.format(lateral_flux_val))
    ...
    #print('DeltaX : {:.10f}, T_approx(0) : {:.10f}, T_exact(0) : {:.10f},Percent Error : {:.10f}, q_dot_total_approx : {:.10f}, q_dot_tota_exact : {:.10f} , beta : {:.10f}'.format(delta_x,T_approx, T_exact, percent_error, q_dot_env_approx, q_dot_env_exact, beta))
...
delta_x_array = [1/(2**2), 1/(2**3), 1/(2**4), 1/(2**5), 1/(2**6), 1/(2**7), 1/(2**8), 1/(2**9), 1/(2**10), 1/(2**11), 1/(2**12)]
delta_x_array = np.log(delta_x_array)
total_error_list = np.log(np.abs(total_error_list))
print("Delta X")
print(delta_x_array)
print("Total Error")
...
plt.title("DeltaX vs Beta 10th order convergence for case 2")
plt.plot(delta_x_array[1:9], np.log(total_beta_list[1:9]))
plt.ylabel('Beta')
plt.xlabel('DeltaX')
plt.show()

plt.title("Convergence plot for 10th order for case 2")
plt.plot(delta_x_array, total_error_list)
